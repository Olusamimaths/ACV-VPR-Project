#!/usr/bin/env python3
"""
Enhanced Campus Dataset VPR Test with VPRTempo Support

Extends the original test_campus_dataset.py to support VPRTempo descriptor
while maintaining all original functionality.

Features:
  • Original descriptors (CosPlace, NetVLAD, etc.)
  • New VPRTempo and VPRTempoQuant support
  • Optional temporal aggregation
  • Same evaluation metrics
  • Comparable results with other descriptors
"""
...
def main():
    ...
    parser.add_argument(
        ...
        choices=['HDC-DELF', 'AlexNet', 'NetVLAD', 'PatchNetVLAD',
                 'CosPlace', 'EigenPlaces', 'SAD', 'VPRTempo', 'VPRTempoQuant'],
    ...
    parser.add_argument(
        '--use_temporal',
        action='store_true',
        help='Enable temporal aggregation (VPRTempo only)'
    )
    ...
    if args.use_temporal:
        print(f'Temporal Mode: Enabled (window_size={args.temporal_window_size})')
    print('='*70)
    ...
    if args.descriptor == 'VPRTempo':
        try:
            from live_vpr.extractors import create_feature_extractor
            feature_extractor = create_feature_extractor('VPRTempo')
            print('  ✅ VPRTempo (fp32) extractor loaded')
        except ImportError:
            print('  ❌ ERROR: vprtempo not installed. Install with: pip install vprtempo')
            sys.exit(1)
    
    elif args.descriptor == 'VPRTempoQuant':
        try:
            from live_vpr.extractors import create_feature_extractor
            feature_extractor = create_feature_extractor('VPRTempoQuant')
            print('  ✅ VPRTempoQuant (int8) extractor loaded')
        except ImportError:
            print('  ❌ ERROR: vprtempo not installed. Install with: pip install vprtempo')
            sys.exit(1)
    
    elif args.descriptor == 'HDC-DELF':
    ...
    if args.descriptor not in ['PatchNetVLAD', 'SAD']:
        ...
        if args.descriptor in ['VPRTempo', 'VPRTempoQuant']:
            from live_vpr.extractors import compute_global_descriptors
            db_D_holistic = compute_global_descriptors(feature_extractor, imgs_db)
        else:
            db_D_holistic = feature_extractor.compute_features(imgs_db)
        
        extract_db_time = time.time() - start_time
        print(f'  Database extraction time: {extract_db_time:.2f}s ({extract_db_time/len(imgs_db):.3f}s/image)')
        ...
        if args.descriptor in ['VPRTempo', 'VPRTempoQuant']:
            from live_vpr.extractors import compute_global_descriptors
            q_D_holistic = compute_global_descriptors(feature_extractor, imgs_q)
        else:
            q_D_holistic = feature_extractor.compute_features(imgs_q)
        
        extract_q_time = time.time() - start_time
        print(f'  Query extraction time: {extract_q_time:.2f}s ({extract_q_time/len(imgs_q):.3f}s/image)')
    ...
    title = f'Precision-Recall Curve - Campus Dataset\n{args.descriptor}'
    if args.use_temporal:
        title += ' (Temporal Mode)'
    ...
    if args.save_results:
        ...
        with open(results_file, 'w') as f:
            ...
            if args.use_temporal:
                f.write(f'Temporal Mode: Enabled (window={args.temporal_window_size})\n')
