...
\begin{itemize}[leftmargin=*]
  \item The largest night bucket is \textbf{Place 2} (\texttt{red\_entry\_transition\_zone}) with \textbf{25} night images.
  \item Some places have very little support, such as \texttt{innovation\_hub\_door} and \texttt{banner\_window\_corner}.
  \item \texttt{hallway\_corridor} has day images but no confident night counterpart in the place-level rematch.
\end{itemize}

This uneven coverage means that some locations are much more likely to dominate the matching statistics than others.
