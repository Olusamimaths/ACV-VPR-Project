...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, align=center, minimum width=36mm, minimum height=11mm, fill=boxgray},
  arrow/.style={-{Latex[length=2.5mm]}, thick}
]
\node[box, fill=boxblue] (datasets) {Dataset loaders\\\file{datasets/load\_dataset.py}};
\node[box, fill=boxgreen, right=of datasets] (features) {Descriptor extraction\\\file{feature\_extraction/}};
\node[box, fill=boxorange, right=of features] (matching) {Matching + metrics\\\file{matching/}, \file{evaluation/}};

\node[box, fill=boxblue, below=14mm of datasets] (capture) {Capture / sources\\\file{live\_vpr/sources.py}};
\node[box, fill=boxgreen, right=of capture] (mapbuild) {Map building + storage\\\file{live\_vpr/offline.py}, \file{live\_vpr/database.py}};
\node[box, fill=boxorange, right=of mapbuild] (online) {Online localization + UI\\\file{live\_vpr/online.py}, \file{live\_vpr/ui.py}};

\draw[arrow] (datasets) -- node[above, font=\small] {benchmark path} (features);
\draw[arrow] (features) -- (matching);

\draw[arrow] (capture) -- node[above, font=\small] {live path} (mapbuild);
\draw[arrow] (mapbuild) -- (online);

\node[draw, rounded corners, fit=(datasets) (features) (matching), inner sep=5pt, label=above:{\textbf{Benchmark evaluation pipeline}}] {};
\node[draw, rounded corners, fit=(capture) (mapbuild) (online), inner sep=5pt, label=below:{\textbf{Live deployment pipeline}}] {};
\end{tikzpicture}
}
\caption{Repository overview: two main paths share descriptor extraction ideas, but only the benchmark path computes evaluation metrics during runtime.}
\end{figure}
...
\begin{tikzpicture}[
  node distance=8mm and 8mm,
  box/.style={draw, rounded corners, align=center, minimum width=28mm, minimum height=10mm, fill=boxblue},
  arrow/.style={-{Latex[length=2.5mm]}, thick}
]
\node[box] (entry) {\file{demo.py}\\or \file{test\_campus\_dataset.py}};
\node[box, right=of entry] (dataset) {Load dataset\\images + GT matrices};
\node[box, right=of dataset, fill=boxgreen] (desc) {Extract descriptors\\(global / patch)};
\node[box, right=of desc] (sim) {Similarity matrix $S$\\cosine or local matcher};
\node[box, right=of sim, fill=boxorange] (match) {Matching\\$M_1$, $M_2$};
\node[box, below=of sim, minimum width=42mm, fill=boxgray] (eval) {PR curve, AUC, $R@100P$, $R@K$\\and TP/FP visualization};

\draw[arrow] (entry) -- (dataset);
\draw[arrow] (dataset) -- (desc);
\draw[arrow] (desc) -- (sim);
\draw[arrow] (sim) -- (match);
...
\begin{tikzpicture}[
  node distance=8mm and 8mm,
  box/.style={draw, rounded corners, align=center, minimum width=28mm, minimum height=10mm, fill=boxgreen},
  io/.style={draw, rounded corners, align=center, minimum width=28mm, minimum height=10mm, fill=boxgray},
  arrow/.style={-{Latex[length=2.5mm]}, thick}
]
\node[io] (source) {camera index / alias / URL / video};
\node[box, right=of source] (record) {optional traversal recording\\\file{live\_vpr/capture.py}};
\node[box, right=of record] (sample) {sample frames from video\\\file{sample\_video\_to\_frames}};
\node[box, right=of sample] (map) {build \file{ReferenceMap} (.npz)\\\file{live\_vpr/offline.py}};
\node[box, below=of map] (online) {localize each frame\\\file{live\_vpr/online.py}};
\node[box, left=of online, minimum width=34mm, fill=boxorange] (ui) {overlay + report export\\\file{live\_vpr/ui.py}};

\draw[arrow] (source) -- (record);
\draw[arrow] (record) -- (sample);
\draw[arrow] (sample) -- (map);
\draw[arrow] (map) -- (online);
\draw[arrow] (source) |- (online);
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, align=left, minimum width=45mm, minimum height=10mm, fill=white},
  main/.style={draw, rounded corners, align=center, minimum width=45mm, minimum height=12mm, fill=boxblue},
  arrow/.style={-{Latex[length=2.5mm]}, thick}
]
\node[main] (cli) {\file{live\_vpr\_test.py}\\CLI orchestration};
\node[box, below left=of cli, fill=boxgreen] (offline) {\file{live\_vpr/offline.py}\\map build from folder or sampled video frames};
\node[box, below=of cli, fill=boxgreen] (capture) {\file{live\_vpr/capture.py}\\record traversal + sample timestamps};
\node[box, below right=of cli, fill=boxgreen] (online) {\file{live\_vpr/online.py}\\descriptor + similarity + threshold decision};
\node[box, below=of capture, fill=boxorange] (db) {\file{live\_vpr/database.py}\\ReferenceMap schema and .npz serialization};
\node[box, left=of db, fill=boxorange] (sources) {\file{live\_vpr/sources.py}\\source probing, aliases, and OpenCV wrappers};
\node[box, right=of db, fill=boxorange] (ui) {\file{live\_vpr/ui.py}\\real-time HUD and saved inference report canvases};

\draw[arrow] (cli) -- (offline);
\draw[arrow] (cli) -- (capture);
\draw[arrow] (cli) -- (online);
\draw[arrow] (offline) -- (db);
\draw[arrow] (capture) -- (db);
\draw[arrow] (sources) -- (capture);
\draw[arrow] (sources) |- (online);
\draw[arrow] (db) -- (online);
\draw[arrow] (online) -- (ui);
\end{tikzpicture}
}
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, align=center, minimum width=37mm, minimum height=10mm, fill=boxgray},
  arrow/.style={-{Latex[length=2.5mm]}, thick}
]
\node[box, fill=boxblue] (day) {Reference images\\\file{custom\_dataset/day\_images}};
\node[box, right=of day, fill=boxgreen] (mapnpz) {Saved map artifact\\\file{artifacts/live\_maps/*.npz}};
\node[box, right=of mapnpz, fill=boxorange] (runtime) {Runtime localization\\\file{live\_vpr/online.py}};

\node[box, below=of day] (video) {Traversal video\\\file{artifacts/reference\_videos/*.mp4}};
\node[box, right=of video] (frames) {Sampled frames\\\file{artifacts/reference\_captures/*}};
\node[box, right=of frames] (reports) {Inference outputs\\\file{artifacts/inference\_stats/*}\\\file{artifacts/live\_captures/*}};

\draw[arrow] (day) -- (mapnpz);
\draw[arrow] (mapnpz) -- (runtime);
\draw[arrow] (video) -- (frames);
\draw[arrow] (frames) |- (mapnpz);
...
\textbf{How it works.} Recording and sampling are intentionally separate. That makes it easy to rebuild a map later with a different sampling rate.
...
\textbf{How it works.} The live phase still performs VPR in the same conceptual way as the benchmark path. The difference is that it localizes one frame at a time instead of evaluating an entire query set.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Live flow}: each frame gets one descriptor, then one vector-matrix multiply against stored descriptors.
...
\centering
load map once $\rightarrow$ process frame $\rightarrow$ compute similarities $\rightarrow$ threshold decision $\rightarrow$ visualize/export
}}
\end{center}
...
\centering
image $\rightarrow$ descriptor $\rightarrow$ similarity $\rightarrow$ decision
}}
\end{center}
...
\begin{itemize}[leftmargin=*]
  ...
  \item each TikZ figure is wrapped in \texttt{\textbackslash resizebox\{0.98\textbackslash textwidth\}\{!\}\{...\}} to prevent clipping,
  \item node widths and spacing are bounded so no text spills outside the page box.
\end{itemize}

If a renderer still hides TikZ graphics, compile with \texttt{pdflatex} or \texttt{xelatex} directly and ensure TikZ is enabled in the LaTeX distribution.
