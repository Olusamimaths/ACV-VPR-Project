...
class CosPlaceFeatureExtractor(torch.nn.Module):
    ...
    def compute_features(self, imgs: List[np.ndarray]) -> np.ndarray:
        ...
        # Live inference often runs on a single frame at a time. Avoid spawning
        # DataLoader workers in that case because the startup overhead dominates
        # the actual forward pass and can make inference look CPU-bound.
        if len(imgs) <= 8:
